...
(function() {
    ...
    function sync_edits() {
        ...
        boundary = '-----------------------------' +
            Math.floor(Math.random() * Math.pow(10, 8));
        fname = db.replace(/\//g, "+") + '+' + record.replace(/\//g, "+") + '.' + ann[i].name + '.log';
        body = '--' + boundary + '\r\nContent-Disposition: form-data; name="file";' + ' filename="' + fname + '"\r\nContent-type: text/plain\r\n\r\n' + '[LWEditLog-1.0] Record ' + db + '/' + record + ', annotator ' + ann[i].name + ' (' + tickfreq + ' samples/second)\r\n\r\n' + etext + '\r\n--' + boundary + '--\r\n';
    ...
    function show_plot() {
        ...
        svg = '<br><svg xmlns=\'http://www.w3.org/2000/svg\'' + ' xmlns:xlink=\'http://www.w3.org/1999/xlink\' class="svgplot"' + ' width="' + width + '" height="' + height + '" preserveAspectRatio="xMidYMid meet">\n';
        svg += '<g id="viewport" transform="scale(' + width / svgtw + '),translate(' + svgl + ',' + svgf + ')">\n';
        ...
        for (ia = 0; ia < nann; ia++) {
            ...
            if (ann[ia].state > 0) {
                ...
                if (ann[ia].state === 2) {
                    ...
                    if (editing) {
                        sva += '<rect x="0" y="' + Number(ytop - svgf) + '" width="' + svgw + '" height="' + 3 * svgf + '" fill="#88f" fill-opacity="0.2" />';
                    }
                } else {
                ...
                sva += '<title>' + ann[ia].desc + ' (click for highlighted view)</title>' + '<rect x="-' + svgl + '" y="' + ytop + '" width="' + svgl + '" height="' + 2 * svgf + '" fill="white" />' + '<text x="-50" y="' + y0 + '"' + ' font-size="' + svgf + '" fill="rgb(150,150,200)" font-style="italic"' + ' style="text-anchor: end; dominant-baseline: middle">' + ann[ia].name + '</text></g>\n';
            }
        }
    ...
    function show_time(x, y) {
        ...
        if (y != 0 && editing) {
            xc = x_cursor - 2 * adx4;
            svc = '<path stroke="rgb(0,150,0)" stroke-width="' + lwn + '" fill="none" style="cursor:pointer" d="M' + x_cursor + ',0 l-' + adx2 + ',-' + ady1 + ' l' + adx4 + ',0 l-' + adx2 + ',' + ady1 + ' V' + svgh + ' l-' + adx2 + ',' + ady1 + ' l' + adx4 + ',0 l-' + adx2 + ',-' + ady1 + '" />';
    ...
    function highlight_selection() {
        ...
        if (selann >= 0) {
            ...
            if (0 <= dt && dt <= dt_ticks) {
                ...
                y0 = asy0 - 2 * svgf;
                svsa = '<path stroke="rgb(0,0,0)" stroke-width="' + lwn + '" fill="yellow" fill-opacity="0.2" d="M' + x0 + ',' + y0 + ' l' + 2 * svgf + ',0 l0,' + 3 * svgf + ' l-' + 2 * svgf + ',0 l0,-' + 3 * svgf + '" />';
            }
        }
    }
    ...
    function highlight_phantom(t) {
        ...
        if (0 <= dt && dt <= dt_ticks) {
            x0 = Math.floor(dt * tscl / tickfreq) - svgf;
            y0 = asy0 - 2 * svgf;
            svsa = '<path stroke="rgb(0,0,0)" stroke-width="' + lwn + '" stroke-dasharray="2" fill="white" fill-opacity="0.2" d="M' + x0 + ',' + y0 + ' l' + 2 * svgf + ',0 l0,' + 3 * svgf + ' l-' + 2 * svgf + ',0 l0,-' + 3 * svgf + '" />';
        }
    }
    ...
    function parse_url() {
        ...
        for (n = 0; n < q.length; n++) {
            ...
            if (v[0] === 'db') {
                ...
                server = decodeURIComponent(v[1]);
            } else if (v[0] === 'scribe') {
                scribe = decodeURIComponent(v[1]);
            }
        }
